...
const formStyles = {
    ...
    "userGroup": "BetaSignUp"
}
const domain = "app.loops.so"

export default function SignUpRicochet() {
    ...
    switch (formState) {
        ...
        default:
            return (
                <>
                    <div className="mt-8 relative mx-auto my-auto cursor-default">
                        <form autoComplete="off" onSubmit={handleSubmit} className="flex flex-col items-left justify-center">
                            <div className="flex gap-2 mb-3 w-full">
                                <input autoComplete="off" name="email" type="text" placeholder={formStyles.placeholderText} value={email} onChange={(e) => setEmail(e.target.value)} required={true} aria-label="Email address" className="rounded-lg w-full h-12 bg-[#F1F1F1] px-3 border border-[#C7C7C7]/80 py-[calc(theme(spacing.2)-1px)] shadow-md shadow-zinc-800/5 placeholder:text-zinc-400 focus:outline-none text-black m:text-sm" />
                            </div>
                            <SignUpFormButton />
                        </form>
                    </div>
    ...
    function SignUpFormButton({ props }: any) {
        return (
            <button className="ricochet-primary-button flex bg-[#D3D3D3] shadow-[0px_1px_0px_0px_rgba(255,_255,_255,_0.04)_inset] p-4 h-12 mx-auto w-full border-0 justify-center rounded-lg backdrop-blur-sm text-[#292A2B] items-center font-medium text-lg border-gray-200"
                type="submit">
